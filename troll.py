...
TERMINAL_AB = {}
def endgame_ab(brd, tkn, lower, upper, top=False):  # alpha-beta pruning for negamax
    ...
    if not moves:  # pass
        ...
        ab = endgame_ab(brd, enemy, -upper, -lower)                                # recur
    ...
    for mv in sorted(moves, key=lambda m: -rateMove(brd, tkn, m)):  # sort moves to improve chances of pruning
        new_brd = makeMove(brd, tkn, mv)
        ab = endgame_ab(new_brd, enemy, -upper, -lower)  # recur
        score = -ab[0]
...
def quickMove(brd, tkn):
    ...
    if brd.count('.') < HL: return endgame_ab(brd.lower(), tkn.lower(), TERMINAL_MIN, TERMINAL_MAX, True)[-1]
...
def main():
    ...
    if brd.count('.') < HL:
        ab = endgame_ab(brd.lower(), tkn.lower(), TERMINAL_MIN, TERMINAL_MAX, True)
