...
def quickMove(brd, tkn):
    ...
    psbl_moves = find_moves(brd, tkn)

    viable_moves = [move for move in psbl_moves if not (move in ADJ_CORNERS and ADJ_CORNERS[move] != '.')]
    for move in viable_moves:
        if move in CORNERS: return move
        new_brd = make_move(brd, tkn, move)
        for edge in EDGES:
            if move not in edge: continue
            if [board[i] for i in edge].count('.') == 1:
                return move
            if new_brd[edge[0]: move].count(tkn) == move-edge[0] or new_brd[move: edge[-1]].count(tkn) == edge[-1]-move:
                return move

    return random.choice(viable_moves) if viable_moves else random.choice([*psbl_moves])


def make_move(brd, tkn, move):
...
def set_globals():
    ...
    EDGES = [[1, 2, 3, 4, 5, 6], [8, 16, 24, 32, 40, 48], [57, 58, 59, 60, 61, 62], [55, 47, 39, 31, 23, 15]]
    CORNERS = (0, 7, 56, 63)
...
def main():
    ...
    for move in move_sequence:
        ...
        brd, tkn = make_move(brd, tkn, move), ENEMY[tkn]
        psbl_moves = find_moves(brd, tkn)
        two_d_print(brd, psbl_moves)
        print()
        print(f'{brd} {brd.lower().count(x)}/{brd.lower().count(o)}')
