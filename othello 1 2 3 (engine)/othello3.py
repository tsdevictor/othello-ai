...
def set_globals():
    ...
    global X, O, BW, POSITIONS, BOARD, TOKEN_TO_PLAY, MOVE_SEQUENCE
    ...
    for arg in args:
        ...
        elif '-' not in arg: MOVE_SEQUENCE.append(POSITIONS[arg.lower()])
...
def main():
    ...
    if not moves:
        TOKEN_TO_PLAY = 'x' if TOKEN_TO_PLAY == 'o' else 'o'
        moves = possible_moves(BOARD, TOKEN_TO_PLAY)
        print(print(f'Possible moves for {TOKEN_TO_PLAY}: {str(moves)[1:-1]}'))
    else: print(f'Possible moves for {TOKEN_TO_PLAY}: {str(moves)[1:-1]}')
