...
def findMoves(brd, tkn):
    key = brd + tkn
    # if key in MOVES_CACHE: return MOVES_CACHE[key]
...
def negamax(brd, tkn):
    ...
    if '.' not in brd or (not moves and not findMoves(brd, enemy)): return [brd.count(tkn)-brd.count(enemy)]
    if not moves:
...
def rate(brd, tkn):
    mobility = 5*(len(findMoves(brd, tkn))-len(findMoves(brd, ENEMY[tkn])))
    pieces = brd.count(tkn) - brd.count(ENEMY[tkn])
    return mobility+pieces
...
def quickMove(brd, tkn):
    ...
    if brd.count('.') < N:
        return negamax(brd, tkn)[-1]

    psbl_moves = findMoves(brd, tkn)
    for move in psbl_moves:
        best_move, rating = move, rate(makeMove(brd, tkn, move), tkn)
    ...
    for move in psbl_moves:
        ...
        for edge_indices in EDGES:
            ...
            if len(set(edge[:edge_indices.index(move)+1])) == 1 or len(set(edge[edge_indices.index(move):])) == 1:
                return move
        if not (move in ADJ_CORNERS and ADJ_CORNERS[move] != tkn):
            new_rating = rate(brd, tkn)
...
def set_globals():
    ...
    global ENEMY, EDGES, POSITION_VALUE, NBRS, CORNERS, ADJ_CORNERS, N, MOVES_CACHE, MAKE_MOVE_CACHE, NEGAMAX_CACHE
