...
def quickMove(brd, tkn):
    ...
    # if brd.count('.') < HL: return alphabeta(brd.lower(), tkn.lower(), MIN, MAX, True)[-1]

    brd, tkn = brd.lower(), tkn.lower()
    moves = findMoves(brd, tkn)

    return sorted(moves, key=lambda m: -rateMove(brd, tkn, m))[0]


def set_globals():
    # noinspection PyGlobalUndefined
    global HL, ENEMY, MIN, MAX
...
def main():
    ...
    if brd.count('.') < HL:
        ab = alphabeta(brd.lower(), tkn.lower(), MIN, MAX, True)
