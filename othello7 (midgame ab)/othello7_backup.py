...
def makeMove(brd, tkn, mv):
    ...
    for drt in NBRS_BY_DIRECTION[mv]:  # loop through each direction
...
def findMoves(brd, tkn):
    ...
    for cdt in range(64):  # candidate moves
        ...
        for drt in NBRS_BY_DIRECTION[cdt]:  # loop through each direction
...
def alphabeta(brd, tkn, lower, upper, top=False):  # alpha-beta pruning for negamax
    ...
    for mv in sorted(moves, key=lambda m: -rateMove(brd, tkn, m)):
        ...
        ab = alphabeta(new_brd, enemy, -upper, -lower)
        score = -ab[0]
    ...
    for i in range(64):
        ...
        if brd[i] == tkn:
            ...
            for d in DIRECTION_PAIRS:  # stable tkn
                ...
                score -= 1.5
    score += len(findMoves(brd, tkn)) - len(findMoves(brd, enemy))

    BOARD_EVAL[key] = score
    return score


def midgame_ab(brd, tkn, lower, upper, depth, top=False):  # can't cache because different depths have different ratings
    if depth == 0:
        return [board_eval(brd, tkn)]
...
def rateMove(brd, tkn, mv):
    ...
    od = OPPOSITE_DIRECTIONAL_NBR_PAIRS[mv]  # opposite directions
    for d in DIRECTION_PAIRS:  # stable tkn
        if not isTknLine(new_brd, tkn, od[d[0]]) and not isTknLine(new_brd, tkn, od[d[1]]): break
    else: return 40

    if mv in CORNER_NBRS and CORNER_NBRS[mv] != tkn: return -100

    enemy = ENEMY[tkn]
    return -len(findMoves(new_brd, enemy)) * 2
